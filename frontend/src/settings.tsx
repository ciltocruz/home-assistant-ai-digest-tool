...
type FormState = {
  ...
  privacyLevel: EditableSettingsDto['privacyLevel']; retentionDays: string; includeWarnings: boolean;
};
...
export function SettingsPanel({ api, section }: { api?: SettingsApi; section?: string }) {
  ...
  return <section className="panel settings-panel" aria-labelledby="settings-title">
    ...
    <LiveFeedback message={feedback} error={status === 'error' || notesState === 'error' || ignoresState === 'error'} />
     {selectedSection === 'context' && contextError ? <><p className="error-copy" role="alert">{contextError}</p>{retryTarget ? <button type="button" className="secondary-action" onClick={() => setRemoveTarget(retryTarget)}>{copy('settings.retryRemoval')}</button> : null}</> : null}
     {selectedSection !== 'context' ? <form className="control-form settings-form" onSubmit={(event) => void save(event)} noValidate>
       {selectedSection === 'connection' ? <fieldset id="settings-connection"><legend>{copy('settings.homeAssistant')}</legend>
       ...
       {selectedSection === 'ai' ? <fieldset id="settings-ai"><legend>{copy('settings.ai')}</legend>
       ...
       {selectedSection === 'notifications' ? <fieldset id="settings-notifications"><legend>{copy('settings.notifications')}</legend>
       ...
       {selectedSection === 'schedule' ? <fieldset id="settings-schedule"><legend>{copy('settings.schedule')}</legend>
         ...
         <label className="checkbox-label checkbox-switch"><input name="scheduleEnabled" type="checkbox" checked={form.scheduleEnabled} onChange={(event) => update('scheduleEnabled', event.currentTarget.checked)} /><span className="switch-track"><span className="switch-thumb" /></span><span className="checkbox-text">{copy('settings.enableSchedule')}</span></label>
       </fieldset> : null}
       {selectedSection === 'privacy' ? <fieldset id="settings-privacy"><legend>{copy('settings.privacyRetention')}</legend>
         ...
         <label className="checkbox-label checkbox-switch"><input name="includeWarnings" type="checkbox" checked={form.includeWarnings} onChange={(event) => update('includeWarnings', event.currentTarget.checked)} /><span className="switch-track"><span className="switch-thumb" /></span><span className="checkbox-text">{locale === 'es' ? 'Incluir avisos' : 'Include warnings'}</span></label>
       </fieldset> : null}
...
function SecretControls({ copy, label, name, mask, configured, operation, value, onOperation, onValue }: { copy(key: TranslationKey): string; label: string; name: string; mask?: string; configured: boolean; operation: 'keep_current' | 'replace'; value: string; onOperation(operation: 'keep_current' | 'replace'): void; onValue(value: string): void }) {
  return <fieldset className="secret-controls"><legend>{label}</legend><p className="muted-copy">{configured ? copy('settings.configured').replace('{mask}', mask ?? '••••') : copy('settings.notConfigured')}</p><div className="secret-radio-group"><label className={`secret-radio-card ${operation === 'keep_current' ? 'active' : ''}`}><input type="radio" name={`${name}Operation`} checked={operation === 'keep_current'} onChange={() => onOperation('keep_current')} /><span className="radio-indicator"><span className="radio-dot" /></span><span className="radio-text">{copy('settings.keepCurrent')}</span></label><label className={`secret-radio-card ${operation === 'replace' ? 'active' : ''}`}><input type="radio" name={`${name}Operation`} value={`replace-${name === 'aiKey' ? 'ai-key' : name}`} checked={operation === 'replace'} onChange={() => onOperation('replace')} /><span className="radio-indicator"><span className="radio-dot" /></span><span className="radio-text">{copy('settings.replace')}</span></label></div>{operation === 'replace' ? <label className="secret-new-value">{copy('settings.newValue')}<input name={name} type="password" required value={value} onChange={(event) => onValue(event.currentTarget.value)} autoComplete="new-password" /></label> : null}</fieldset>;
}

function toFormState(settings: EditableSettingsDto): FormState { const schedule = settings.schedules[0] ?? { kind: 'daily' as const, enabled: true, time: '08:00', timezone: 'Europe/Madrid' }; return { haUrl: settings.homeAssistant.url, haTokenOperation: 'keep_current', haToken: '', aiProvider: settings.ai.provider, aiKeyOperation: 'keep_current', aiKey: '', notificationChannel: settings.notifications.channel, telegramChatId: settings.notifications.channel === 'telegram' ? settings.notifications.chatId : '', telegramOperation: 'keep_current', telegramBotToken: '', scheduleKind: schedule.kind, scheduleEnabled: schedule.enabled, scheduleTime: schedule.time, timezone: schedule.timezone, dayOfWeek: schedule.kind === 'weekly' ? String(schedule.dayOfWeek) : '1', privacyLevel: settings.privacyLevel, retentionDays: String(settings.retentionDays), includeWarnings: settings.includeWarnings ?? false }; }
