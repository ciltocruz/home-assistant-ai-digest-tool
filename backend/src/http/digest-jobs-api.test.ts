...
describe('durable digest job API', () => {
  ...
  it('returns 202 with a persisted job and exposes safe progress instead of pretending completion', async () => {
    ...
    app = createApp({ services, auth: auth() });
...
function auth() { return { adminToken: 'admin-token', setupToken: 'setup-token', sessionTtlMs: 60_000 }; }
async function login(app: ReturnType<typeof createApp>) {
  const response = await app.inject({ method: 'POST', url: '/api/session', payload: { adminToken: 'admin-token' } });
  return { cookie: response.headers['set-cookie'], csrfToken: response.json<{ csrfToken: string }>().csrfToken };
}
function servicesForJobs(): BackendApiServices {
  const state = { id: 'job-1', triggerWindowId: 'manual:window', kind: 'manual' as const, status: 'running' as const, stage: 'generating' as const, attempts: 0, retryCount: 0, retryAvailable: false, availableAt: '2026-08-01T10:00:00.000Z', createdAt: '2026-08-01T10:00:00.000Z', updatedAt: '2026-08-01T10:00:00.000Z' };
  return {
    ...
    reports: { list: async () => [], get: async () => null }, notes: { add: async () => { throw new Error('unused'); }, listWindow: async () => [] }, ignores: { add: async () => { throw new Error('unused'); }, remove: async () => undefined, listActive: async () => [] }, notifiers: { test: async () => ({ status: 'success', message: 'ok', checkedAt: '2026-08-01T10:00:00.000Z' }), send: async () => ({ status: 'skipped', targetRef: 'ref' }) }
  };
}
